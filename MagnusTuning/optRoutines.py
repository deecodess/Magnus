...
def opt_differential_evolution(parametersList):
    ...
    scipy_res=differential_evolution(Magnus_func,bounds,args=parametersList,maxiter=max_iter,disp=True,polish=False)
    return scipy_res
...
def opt_basinhopping(parametersList):
    ...
    print('Initial guess')
    print(x0)
    scipy_res=basinhopping(Magnus_func,x0,niter=n_iter,T=50,stepsize=10, minimizer_kwargs=args,niter_success=3,disp=True)
    return scipy_res
